...
@section('content')
    ...
    <form action="{{ route('staff.staffSchedule.update', $staffSchedule->id) }}" method="post" autocomplete="off" id="form">
        @csrf
        @method('PATCH')
        <div class="card shadow mb-4">
            <div class="card-header py-3">
                <div class="row">
                    <div class="col-6">
                        <a href="{{ route('staff.staffSchedule.index') }}" class="btn btn-warning">
    ...
    <form id="setDefault" action="{{ route('staff.staffSchedule.makeDefault', $staffSchedule->id) }}" method="post">
        @csrf
    </form>

    <!-- Calendar -->

<form action="{{ route('staff.staffSchedule.hour') }}" method="POST" id="calendar_form">
    @csrf
    <input type="hidden" name="method" value="{{ $method }}" />
    <input type="hidden" name="staff_schedule_id" value="{{ $staffSchedule->id }}" />
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <div class="row">
                <div class="col-6">
            ...
            <table class="table table-hover" id="datatable">
                ...
                <tbody>
                    ...
                    @foreach($staffSchedule->days as $day)
                        ...
                        <tr>
                            <td>
                                <form action="{{ route('staff.staffSchedule.changeDayStatus', $day->id) }}" method="post" id="status-{{ $day->id }}">
                                    @csrf
